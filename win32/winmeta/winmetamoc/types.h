...
inline std::string make_struct_type(const ElementType& t)
{
    ...
    if (typenames.count(t))
    {
        //std::cout << "T:" << typenames[t] << std::endl;
    }
    if (theMap.count(t) == 0)
    {
        return "ptr";
...
std::pair<std::string, std::string> fieldType(const Field& field)
{
    std::string entry(field.Name());
    ...
    else
    {
        ...
        auto as = t.array_sizes();

        if (t.ptr_count() > 0)
        {
            if ((t.ptr_count() == 1) && (t.element_type() == ElementType::Char))
            {
                typeName = "wstr";
            }
            else if ((t.ptr_count() == 1) && (t.element_type() == ElementType::U8))
            {
                typeName = "str";
            }
        ...
        {
            std::string n = get_type(t);
            if (!n.empty())
            {
                if (n == "Windows.Win32.Foundation.PSTR") typeName = "str";
                else if (n == "Windows.Win32.Foundation.PWSTR") typeName = "wstr";
                else typeName = n;
            }
            else
            {
    ...
    return std::make_pair(entry, typeName);
}


std::string isSimpleStruct(TypeDef& typeDef)
{
...
inline std::string get_win32_type(const TypeSig& typeSig)
{
    std::string typeName;
    if (typeSig.ptr_count() > 0)
    {
        if (typeSig.element_type() == ElementType::Char)
        {
            typeName = "wstr";
        }
        else
    ...
    std::string n = get_type(typeSig);
    if (n == "Windows.Win32.Foundation.PWSTR")
    {
        return "wstr";
    }

    size_t pos = n.find(".");
    if (pos == std::string::npos && !n.empty())
    {
        return n;
    }
    ...
    if (pos != std::string::npos)
    {
        TypeDef type = theCache.find(n);
        if (type)
        {
            auto cat = get_category(type);
            if (cat == category::enum_type)
            {
                return get_enum_base(type);
            }
            auto simple = isSimpleStruct(type);
            if (!simple.empty())
            {
                return simple; 
            }
    ...
    //std::cout << "[" << typeSig.ptr_count() << " " << get_type(typeSig) << "] ";

    typeName = make_struct_type(typeSig.element_type());
    
    return typeName;
}

inline std::string get_import(const TypeDef& typeDef, const MethodDef& methodDef)
{
    std::string result;

    auto i = methodDef.coded_index<MemberForwarded>().index();

    auto mfwd = methodDef.index();// .coded_index<MemberForwarded>();

    auto impls = methodDef.get_database().ImplMap;
    for (ImplMap impl : impls)
    {
       // auto impl = impls[mfwd];
        auto idx = impl.get_value<uint32_t>(1) -1;

       // auto md = methodDef.get_database().MethodDef[idx];

        auto s2 = impl.get_database().get_string(impl.get_value<uint32_t>(2));

//        std::cout << methodDef.Name() << " =?= " << s2 << std::endl;

        //if (idx != mfwd) continue;
        if (s2 != methodDef.Name()) continue;

        //std::cout << methodDef.Name() << " =?= " << s2 << std::endl;


        auto imp = impl.get_value<uint32_t>(3) -1;
        
        auto mod = impl.get_database().ModuleRef[imp].get_value<uint32_t>(0);
        //auto mod = methodDef.get_database().Module[imp].get_value<uint32_t>(1);

//        methodDef.get_database().mr

        auto s = impl.get_database().get_string(mod);
        //std::cout << i << " / " << idx << ": " << s << std::endl;
